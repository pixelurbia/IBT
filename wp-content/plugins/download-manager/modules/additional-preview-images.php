...
<div id="adpcon">
    <a id="add-prev-img" href="#" style='float:left;margin:3px;padding:3px;width: 68px; height: 68px;border: 2px dashed #ddd;background: url(<?php echo WPDM_BASE_URL; ?>images/plus.png) no-repeat center center'></a>
<?php
  ...
  jQuery('body').on('click', '#add-prev-img', function( event ){
    ...
    file_frame.on( 'select', function() {
      ...
      var newDate = new Date;
        ...
        jQuery('#adpcon').append("<div id='"+ID+"' style='display:none;float:left;margin:3px;padding:5px;height:68px;width:68px;background: url("+imgurl+") no-repeat;background-size:cover;' class='adp'><input type='hidden' id='in_"+ID+"' name='file[additional_previews][]' value='"+imgurl+"' /><nobr><b><img style='position:absolute;z-index:9999;cursor:pointer;' id='del_"+ID+"' src='<?php echo plugins_url(); ?>/download-manager/images/minus.png' rel='del' align=left /></b></nobr><div style='clear:both'></div></div>");
        jQuery('#'+ID).fadeIn();
