...
function wpdm_initiate_settings()
{
    global $stabs;
    $tabs = array();
    $tabs['basic'] = array('id' => 'basic','icon'=>'fa fa-cog', 'link' => 'edit.php?post_type=wpdmpro&page=settings', 'title' => 'Basic', 'callback' => 'basic_settings');
    $tabs['frontend'] = array('id' => 'frontend','icon'=>'fa fa-desktop', 'link' => 'edit.php?post_type=wpdmpro&page=settings&tab=frontend', 'title' => 'Frontend', 'callback' => 'wpdm_frontend_settings');
    if (function_exists('bp_is_active')) {
        $tabs['buddypress'] = array('id' => 'buddypress','icon'=>'fa fa-users', 'link' => 'edit.php?post_type=wpdmpro&page=settings&tab=buddypress', 'title' => 'BuddyPress', 'callback' => 'buddypress_addon_settings');
    }

    if(defined('WPDM_CLOUD_STORAGE')){
        $tabs['cloud-storage'] = array('id' => 'cloud-storage','icon'=>'fa fa-cloud',  'link' => 'edit.php?post_type=wpdmpro&page=settings&tab=cloud-storage', 'title' => 'Cloud Storage', 'callback' => 'wpdm_cloud_storage_settings');
    }
    ...
    $stabs['license'] = array('id' => 'license','icon'=>'fa fa-key',  'link' => 'edit.php?post_type=wpdmpro&page=settings&tab=license', 'title' => 'License', 'callback' => 'wpdm_licnese');
...
function wpdm_frontend_settings(){
    if(isset($_POST['section']) && $_POST['section']=='frontend' && isset($_POST['task']) && $_POST['task']=='wdm_save_settings' && current_user_can('manage_options')){
        foreach($_POST as $k => $v){
            if(strpos("__".$k, '_wpdm_')){
                update_option($k, $v);
            }
    ...
    include("admin/settings/frontend.php");
}
...
function ImportFiles()
{
    ...
    include dirname(__FILE__) . '/admin/wpdm-import.php';
...
function LinkTemplates()
{
    ...
    if (isset($_GET['task']) && ($_GET['task'] == 'EditTemplate' || $_GET['task'] == 'NewTemplate')) {
        include("admin/wpdm-template-editor.php");
    } else
        include("admin/wpdm-link-templates.php");
}
